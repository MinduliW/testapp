function tourTestOutcome(x, debrisID, param)

param.t0 = x(end); %juliandate(datevec,'dd-mmm-yyyy');
...
RAANf_t0 = debris(i,4)-param.k*(param.mu/af)^3.5*cos(incf)*(sum(TOFs)*param.TU);

endentry = i + (k -1);
startentry = endentry-1;

[Tdv,TOF,Omega,waittime,rr] = propOpt(x(startentry:endentry), a0,inc0,RAAN0 , af, incf,...
    RAANf_t0, param);

% rrs = [rrs rr];
% figure;
% plot3(rrs(1,:), rrs(2,:),rrs(3,:), 'ro');
% 
...
plot(tSeg/param.TU, af/1e3*ones(size(tSeg)),'--');
p = plot(tSeg/param.TU,semiMajor/1e3);
plot_latex(p, 'time(days)', 'a (km)','', 'Leg 3: From Debris to orbit below ISS' ,{'$a_0$','$a_f$','$a(t)$'});
