function [x] = CoptSolve(As,Bs,constants,xg,N,TR, param)
...
% probc  = [state(6*(N+1)), u(3*N), eta(N), v(6*N), vic(6), vfc(6)] 
prob.c   = [zeros(1,6*(N+1)+3*N), ones(1, N),ones(1,6*N), ones(1,2*6)]; %+...
%0.1*[zeros(1,6*(N+1)+3*N), zeros(1, N),];
%+ ...
...
prob.bux  = [prob.bux , xg(end,1:3)+limits(i,1:3),xg(end,4:6)+limits(i,4:6)];
prob.blx = [prob.blx, xg(end,1:3)-limits(i,1:3),xg(end,4:6)-limits(i,4:6)];
